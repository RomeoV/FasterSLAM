...
#else
    __m256d pht = _mm256_add_pd( _mm256_mul_pd(p1133, h1313), _mm256_mul_pd( p0022, h0202 ) );
#endif
...
#else
    s = _mm256_add_pd( _mm256_mul_pd(h0022, pht0101), s );
#endif
#ifdef __FMA__
    s = _mm256_fmadd_pd( h1133, pht2323, s );
#else
    s = _mm256_add_pd( _mm256_mul_pd(h1133, pht2323), s );
#endif
    _mm256_store_pd( S, s );
...
#else
    __m256d w = _mm256_add_pd( _mm256_mul_pd(pht1133, sinv2323), _mm256_mul_pd( pht0022, sinv0101 ) ); 
#endif
    _mm256_store_pd(W, w);
...
#else
    __m256d w1w1t = _mm256_add_pd( _mm256_mul_pd(w1133, pht1313), _mm256_mul_pd( w0022, pht0202 ) );
#endif
