...
int main() {
    "add feature"_test = [] {
        given("A particle with three known features and vehicle position {1,1,acos(4/5)}) "  // the angle is so that we can have a triangle with 3,4,5 side lengths for the landmark
            ...
            Vector2d landmarks[2] = {{5,0}, {2, M_PI/2 - acos(4./5.)}};  // local robot frame

            Matrix2d R = {0.1*0.1, 0, 0, (1.0*M_PI/180)*(1.0*M_PI/180)};  // this is from the yglee config
            ...
            when("I add the new features to the particle") = [&]() mutable {
                ...
                then("I expect there to be two more features and the same number of max features") = [&] {
                    "Nfa"_test = [&] {
