...
inline void vTMv_base(double* Ms, double* Vs, double* results) {
...
#ifdef __AVX2__
inline void vTMv_avx2(double* Ms, double* Vs, double* results) {
...
#ifdef __AVX2__
inline void vTMv_avx2_phil(double* Ms, double* Vs, double* results) {
    for (size_t i = 0; i+3 < NR; i+=4) {
        __m256d M_intr[4];
        for (size_t j = 0; j < 4; j++) { M_intr[j] = _mm256_load_pd(Ms + 4*i + 4*j); }
        ...
        __m256d avx_results_intr = mm_vT_M_v_avx2_phil(M_intr[0], M_intr[1],
                                                    M_intr[2], M_intr[3],
                                                    v_intr[0], v_intr[1]);
        _mm256_store_pd(results + i, avx_results_intr);
    }
...
int main() {
    ...
    Benchmark<decltype(&vTMv_base)> bench("vTMv_2x2 Benchmark");
    ...
    bench.add_function(&vTMv_avx2_phil, "vTMv avx2 better permutes", 9*NR);
#endif
