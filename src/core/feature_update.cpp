...
void feature_update(Particle* particle,
  ...
  for (size_t i = 0; i < N_idf; i++) {
    copy(particle->xf + (2 * idf[i]), 2, xf[2 * i]);  // means
    copy(particle->Pf + (4 * idf[i]), 4, Pf[4 * i]);  // covariances
  }
  ...
  for (int i = 0; i < N_z; i++) {
    sub(z[2 * i], zp[2 * i], 2, feat_diff[2*i]);
    feat_diff[2*i][1] = pi_to_pi(feat_diff[2*i][1]);
  }
  ...
  for (int i = 0; i < N_idf; i++) {
    ...
    KF_cholesky_update(xf[2 * i], Pf[4 * i], feat_diff[2 * i], R, Hf[4 * i]);
    // xf[i] = xfi;
    // Pf[i] = Pfi;
  }

  for (size_t i = 0; i < N_idf; i++) {
    particle->set_xfi(particle, xf[2 * i], idf[i]);
    particle->set_Pfi(particle, Pf[4 * i], idf[i]);
  }
}
