...
int main() {
    ...
    Vector3d xtrue;
    data_loader(wp, N_waypoints, V, *Q, dt, N, xtrue, &iwp, &G,particles);
    predict_update_VP_active(wp, N_waypoints, V, *Q, dt, N, xtrue, &iwp, &G, particles);
    ...
    Benchmark<decltype(&predict_update_VP_base)> bench("predict_update_VP benchmark");

    bench.data_loader = data_loader;
    // Add your functions to the struct, give it a name (Should describe improvements there) and yield the flops this function has to do (=work)
    // First function should always be the base case you want to benchmark against!
    bench.add_function(&predict_update_VP_base, "base", 0.0);
    bench.add_function(&predict_update_VP_active, "fast", 0.0);
    ...
    Benchmark<decltype(&predict_update_VP_base)> bench_scale("predict_update_VP with Particles");
    bench_scale.add_function(&predict_update_VP_base, "base", 0.0);
    bench_scale.add_function(&predict_update_VP_active, "fast", 0.0);
    //bench_scale.add_function(&predict_update_fast, "fast", 0.0);

    bench_scale.data_loader = data_loader;
    bench_scale.csv_path = "predict_update_VP_scale_particles.csv";
